...
async def weather(ctx, city):
    ...
    # now getting the json data
    data = requests.get(
        f'https://api.openweathermap.org/data/2.5/weather?q={city}&appid=cd0755a868710f3da85d3e63a838080e').json()
    # now getting the mausam
    mausam = data['mausam'][0]['main']
    ...
    # now using the embed to add the mausam, temp, humidity and wind
    embed = discord.Embed(title="Test", description="Test", color=0xe91e63)
    # adding image to embed
    embed.set_image(url=f'https://openweathermap.org/img/wn/{data["mausam"][0]["icon"]}@2x.png')
    embed.add_field(name='Weather', value=mausam)
